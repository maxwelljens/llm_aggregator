# TODO — Architecture Deepening (post-v1.0.2 refactor round)
...
## 1. TUI path drops the result and disables signals
...
## 2. Run-path assembly and exit codes smeared across main.go
...
## 3. LLM client constructed inside `Execute` — seam exists only for tests
...
## 4. Fat 8-method `Progress` interface
...
## 5. Aggregator: ctx ignored, two HTTP stacks, errors swallowed
...
## 6. CLI→config is a stringly `map[string]any` bridge into a god Runtime
...
## 7. Dead code + duplicated constants sweep
